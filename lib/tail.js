...
Tail.prototype.read = function(fd, close) {
  ...
  try {
    ...
    while (index !== -1) {
      ...
      // TODO: use sliding window/token bucket to pause based on bytes per second
      //       if bytes exceeds limit delay processing events. the problem is how that
      //       you need to pause at the event layer. and how do you handle if a thing is deleted etc?
      this.emit("line", line);
